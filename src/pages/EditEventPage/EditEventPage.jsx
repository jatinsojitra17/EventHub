  ...
  return (
    <main className={styles.editEventPage}>
      ...
      <form autoComplete="off" className={styles.editForm} onSubmit={handleSubmit}>
        ...
        { error ? <p className={styles.errorMessage}>{error}</p> : <p>Provide the future date & time when this event will close.</p> }
        <input value={formData.endDate} type="datetime-local" name="endDate" id="endDate" className={styles.dateInput} onChange={handleChange} required /><br />

        <label htmlFor="seats">Seats</label><br />
        <p>How many people can attend this event? Set a limit for the number of attendees.</p>
        <input value={formData.seats} type="number" name="seats" id="seats" placeholder="Number of seats" className={styles.seatsInput} onChange={handleChange} required /><br />
